...
const Profile = () => {
  ...
  const savematch = () => {
  console.log(match)
    commentService.createNote(match).then((match) => {
      setMatch(match);
                     });
        ...
        <div className="table-responsive">
          ...
          <FormControl mb="1rem">
            ...
            <Input
              value={match.matchId}
              onChange={(e) => {
                setMatch({ ...match, matchId: e.target.value });
              }}
            />
          </FormControl>
          <FormControl mb="1rem">
            <FormLabel fontSize="20px">Note</FormLabel>
            <Input
              type="note"
              value={match.note}
              onChange={(e) => {
                setMatch({ ...match, note: e.target.value });
              }}
            />
          </FormControl>
          <Stack direction="column" spacing={4} align="center" pt="2rem">
            <Button colorScheme="green" size="lg" width="xs" onClick={savematch}>
              Savematch
            </Button>
          </Stack>
        </Box>
